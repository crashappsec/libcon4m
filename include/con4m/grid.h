...
extern grid_t *con4m_grid(int start_rows, int start_cols, char *table_tag, char *th_tag, char *td_tag, int header_rows, int header_cols, int stripe);
extern grid_t *grid_horizontal_flow(xlist_t *, uint64_t, uint64_t, char *, char *);
static inline void
grid_set_cell_contents(grid_t *g, int row, int col, object_t item)
{
    ...
    case T_UTF32: {
        ...
        cell = con4m_new(tspec_renderable(), kw("tag", ka(tag), "obj", ka(item)));
        break;
    }
    default:
        abort();
