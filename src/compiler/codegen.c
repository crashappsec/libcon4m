...
{
    // TODO: fixme
    return c4m_gc_alloc_mapped(c4m_zinstruction_t, C4M_GC_SCAN_ALL);
}
...
_emit(gen_ctx *ctx, int32_t op32, ...)
{
    ...
    instr->type_info          = type ? c4m_type_resolve(type) : NULL;
...
gen_literal(gen_ctx *ctx)
{
    ...
    c4m_type_t *ttype = c4m_type_tuple_from_xlist(li->type->items);
...
#define BINOP_ASSIGN_GEN(ctx, op, t)                           \
    if (c4m_type_get_kind(t) == C4M_DT_KIND_primitive) {       \
        if (c4m_type_is_int_type(t)) {                         \
            gen_int_binary_op(ctx, op, c4m_type_is_signed(t)); \
        }                                                      \
                                                               \
