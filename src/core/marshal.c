                      ...
                      c4m_marshal_ctx *ctx)
{
    ...
    c4m_marshal_wl_item_t *rec;

    ctx->memo += hdr->alloc_len;
    rec = c4m_gc_alloc_mapped(c4m_marshal_wl_item_t,
                              C4M_GC_SCAN_ALL);
...
static inline uint64_t
translate_to_fake_pointer(void *pointer, c4m_marshal_ctx *ctx)
{
    ...
    if (lsb_erase((uint64_t)pointer) == lsb_erase((uint64_t)ctx->memo)) {
        c4m_handle_collision(pointer, ctx);
        return 0ULL;
    }
...
static void
write_one_word(void *pointer, c4m_marshal_ctx *ctx, bool always_data)
{
    if (always_data) {
        c4m_marshal_u64((uint64_t)pointer, ctx->s);
    }
    else {
        c4m_marshal_u64(translate_to_fake_pointer(pointer, ctx), ctx->s);
    }
...
{
    uint64_t *p           = ctx->cur->data;
    uint32_t  numwords    = (hdr->alloc_len - sizeof(c4m_alloc_hdr)) / 8;
...
automarshal_one_allocation(c4m_marshal_ctx *ctx)
{
    ...
    assert(sizeof(c4m_marshaled_hdr) == sizeof(c4m_alloc_hdr));
