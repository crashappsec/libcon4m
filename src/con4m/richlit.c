...
parse_style_lit(fmt_frame_t *f, utf8_t *instr)
{
	...
	if (n == 0) {
		...
		case 11:
		    if (!on_ok) {
			CRAISE("'on' keyword in style tag must appear after "
			       "a valid color.");
		    }
		    on_ok        = false;
		    expect_color = true;
		    break;
		default:
		    RAISE(string_concat(s, new_utf8(
			  ": style keyword is for turning off colors. Either "
	    ...
	    if (seen & n) {
		RAISE(string_concat(
			  new_utf8("Duplicate param in style tag: "), s));
	    }
