                                    ...
                                    con4m_arena_t   *new_arena)
{
    ...
    gc_trace("ptr_update_scan:begin:@%p-@%p:%lu", hdr, hdr->next_addr, (size_t)(((char *)hdr->next_addr) - (char *)hdr->data));
        ...
        while (w) {
            ...
            if ((*ploc) >= arena_start && (*ploc) <= arena_end) {
                gc_trace("ptr_update_one:in_heap_yes:@%p:->%p:%zu",
                         ...
                         *ploc,
                         offset);
                process_traced_pointer(ploc, *ploc, arena_start, arena_end, new_arena);
            }
        }
        offset += 64;
    }
    gc_trace("ptr_update_scan:end:@%p-@%p:%lu", hdr, hdr->next_addr, (size_t)(((char *)hdr->next_addr) - (char *)hdr->data));
}
...
static void
process_traced_pointer(uint64_t **addr, uint64_t *ptr, uint64_t *start, uint64_t *end, con4m_arena_t *new_arena)
{
                        ...
                        uint64_t            *stack_bottom)
{
    ...
    for (uint64_t i = 0; i < num_roots; i++) {
        ...
        while (p != stack_bottom) {
            process_traced_pointer((uint64_t **)p, (uint64_t *)*p, start, end, new);
            p++;
        }
    }
}
...
con4m_collect_arena(con4m_arena_t **ptr_loc)
{
    ...
    while (cur != NULL) {
        ...
        con4m_collect_sub_arena(cur, new, roots, num_roots, (uint64_t *)stack_top, (uint64_t *)stack_bottom);
        cur = prior_sub_arena;
    }
