...
static const c4m_ffi_type *ffi_type_map[] = {
    &ffi_type_void,
    &ffi_type_uint8,
    &ffi_type_sint8,
    &ffi_type_uint16,
...
ffi_init()
{
    if (c4m_symbol_cache == NULL) {
        c4m_symbol_cache = c4m_dict(c4m_type_utf8(), c4m_type_ref());
        c4m_gc_register_root(&c4m_symbol_cache, 1);
    }
}

int64_t
c4m_lookup_ctype_id(char *found)
{
...
c4m_ffi_type *
c4m_ffi_arg_type_map(uint8_t ix)
{
    return (c4m_ffi_type *)ffi_type_map[ix];
}

void
c4m_add_static_function(c4m_utf8_t *name, void *symbol)
{
    ffi_init();

    hatrack_dict_put(c4m_symbol_cache, name, symbol);
}
...
c4m_ffi_find_symbol(c4m_utf8_t *name, c4m_list_t *opt_libs)
{
    ...
    void *ptr = hatrack_dict_get(c4m_symbol_cache, name, NULL);
