...
string_slice(const any_str_t *instr, int64_t start, int64_t end)
{
    ...
    utf32_t *res       = c4m_new(tspec_utf32(),
...
string_copy(const any_str_t *s)
{
    ...
    any_str_t *res = c4m_new(u8 ? tspec_utf8() : tspec_utf32(),
...
string_concat(const any_str_t *p1, const any_str_t *p2)
{
    ...
    utf32_t *r           = c4m_new(tspec_utf32(), c4m_kw("length", c4m_ka(n)));
...
_string_join(const xlist_t *l, const any_str_t *joiner, ...)
{
    ...
    utf32_t     *result   = c4m_new(tspec_utf32(),
...
utf8_to_utf32(const utf8_t *instr)
{
    ...
    utf32_t     *outstr = c4m_new(tspec_utf32(), c4m_kw("length", c4m_ka(len)));
...
string_from_int(int64_t n)
{
    ...
    return c4m_new(tspec_utf8(), c4m_kw("cstring", c4m_ka(p)));
}
...
utf8_repeat(codepoint_t cp, int64_t num)
{
    ...
    int     blen   = l * num;
    utf8_t *res    = c4m_new(tspec_utf8(), c4m_kw("length", c4m_ka(blen + 1)));
    char   *p      = res->data;
...
utf32_repeat(codepoint_t cp, int64_t num)
{
    ...
    utf32_t     *res = c4m_new(tspec_utf8(), c4m_kw("length", c4m_ka(num + 1)));
...
err:
    ...
    utf8_t *result = c4m_new(tspec_utf8(), c4m_kw("length", c4m_ka(len)));
    char   *p      = result->data;
...
string_split(any_str_t *str, any_str_t *sub)
{
    ...
    flexarray_t *result = c4m_new(tspec_list(tspec_utf32()),
    ...
    int64_t start = 0;
    int64_t ix    = string_find(str, sub, c4m_kw("start", c4m_ka(start)));
    int     n     = 0;

    while (ix != -1) {
        flexarray_set(result, n++, string_slice(str, start, ix));
        start = ix + subcp;
        ix    = string_find(str, sub, c4m_kw("start", c4m_ka(start)));
    }
...
string_repr(any_str_t *str, to_str_use_t how)
{
    ...
    if (how == TO_STR_USE_QUOTED) {
        utf32_t *q = c4m_new(tspec_utf32(), c4m_kw("cstring", c4m_ka("\"")));
...
{
    if (tspecs_are_compat(target_type, tspec_utf8()) || tspecs_are_compat(target_type, tspec_utf32()) || tspecs_are_compat(target_type, tspec_buffer()) || tspecs_are_compat(target_type, tspec_bool())) {
        return true;
    }
...
string_lit(char *s, syntax_t st, char *litmod, lit_error_t *err)
{
    if (*litmod == 0 || !strcmp(litmod, "u8") || !strcmp(litmod, "utf8")) {
        return c4m_new(tspec_utf8(), c4m_kw("cstring", c4m_ka(s)));
    }

    if (!strcmp(litmod, "u32") || !strcmp(litmod, "utf32")) {
        return c4m_new(tspec_utf32(), c4m_kw("cstring", c4m_ka(s)));
    }
...
const c4m_vtable u8str_vtable = {
    .num_entries = C4M_BI_NUM_FUNCS,
    .methods     = {
        (c4m_vtable_entry)utf8_init,
        (c4m_vtable_entry)string_repr,
        NULL, // finalizer
        (c4m_vtable_entry)c4m_string_marshal,
        (c4m_vtable_entry)c4m_string_unmarshal,
        (c4m_vtable_entry)string_can_coerce_to,
        (c4m_vtable_entry)string_coerce_to,
        NULL, // From lit,
        (c4m_vtable_entry)string_copy,
        (c4m_vtable_entry)string_concat,
        NULL, // Subtract
        NULL, // Mul
        NULL, // Div
        NULL, // MOD
        ...
        NULL, // LT
        NULL, // GT
        (c4m_vtable_entry)string_codepoint_len,
        (c4m_vtable_entry)utf8_index,
        NULL, // Index set
        (c4m_vtable_entry)string_slice,
        NULL, // Slice set
    }};

const c4m_vtable u32str_vtable = {
    .num_entries = C4M_BI_NUM_FUNCS,
    .methods     = {
        ...
        NULL,                        // Div
        NULL,                        // MOD
        (c4m_vtable_entry)string_eq, // EQ
        NULL,                        // LT
        NULL,                        // GT
        (c4m_vtable_entry)string_codepoint_len,
        (c4m_vtable_entry)utf32_index,
